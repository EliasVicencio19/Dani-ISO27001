...
function GapAnalysisScreen() {
  ...
  useEffect(() => {
    ...
    phases[0].questions.forEach((q, i) => {
      if (i === 0) { q.title = tText.q1Title; q.question = tText.q1Question; q.options = [tText.yes, tText.no]; }
      if (i === 1) { q.title = tText.q2Title; q.question = tText.q2Question; q.options = [tText.yes, tText.no]; }
      if (i === 2) { q.title = tText.q3Title; q.question = tText.q3Question; q.options = [tText.yes, tText.no]; }
    });
  ...
  // Render principal - CON BOTONES A LA DERECHA DEL TÍTULO
  return (
    <div style={{ padding: '24px', maxWidth: '1400px', margin: '0 auto', minHeight: '100vh' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px', flexWrap: 'wrap', gap: '16px' }}>
        ...
        {/* Botones a la derecha - EXACTAMENTE COMO EN LA IMAGEN */}
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
          <button 
            ...
            style={{ 
              padding: '8px 20px', 
              borderRadius: '6px', 
              background: activeMainTab === 'assessment' ? '#3b82f6' : 'transparent', 
              border: activeMainTab === 'assessment' ? 'none' : `1px solid ${t.border}`, 
              color: activeMainTab === 'assessment' ? 'white' : t.text, 
              fontWeight: 500, 
              fontSize: '13px', 
              cursor: 'pointer' 
            }}
          ...
          <button 
            onClick={() => setActiveMainTab('soa')} 
            style={{ 
              padding: '8px 20px', 
              borderRadius: '6px', 
              background: activeMainTab === 'soa' ? '#3b82f6' : 'transparent', 
              border: activeMainTab === 'soa' ? 'none' : `1px solid ${t.border}`, 
              color: activeMainTab === 'soa' ? 'white' : t.text, 
              fontWeight: 500, 
              fontSize: '13px', 
              cursor: 'pointer' 
            }}
